package org.max.seminar_5.accu;
...
import org.max.seminar_5.location.Location;
...
public class GetLocationTest extends AbstractTest{
    ...
    void getAdministrativeArea() throws URISyntaxException, IOException {
        ...
        stubFor(get(urlPathEqualTo("/locations/v1/cities/autocomplete"))
                ...
                .willReturn(aResponse()
                        .withStatus(200).withBody("200 OK")));
        CloseableHttpClient httpClient = HttpClients.createDefault();
        HttpGet request = new HttpGet(getBaseUrl()+"/locations/v1/cities/autocomplete");
        URI uri = new URIBuilder(request.getURI())
                .addParameter("apiKey", "A_82c9229354f849e78efe010d94150807")
                .addParameter("q", "data_1")
                .build();
        request.setURI(uri);
        //when
        HttpResponse response = httpClient.execute(request);
        ...
        assertEquals("200 OK", convertResponseToString(response));
    }
}
